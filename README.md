...
- Not in Step 6: application schema, migrations, production roles or RLS policies, the JWT claims hook, API or worker wiring. The approved Phase 2 model for `app_api` is membership in `authenticated` `WITH INHERIT FALSE, SET TRUE`.
...
- `pnpm run lint`
- `pnpm run check:policy` (workspace integrity, workflow policy, install scripts, currency literals) and `pnpm run check:package-age` (needs the npm registry)
