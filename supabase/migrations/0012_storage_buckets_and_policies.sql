...
-- ---------------------------------------------------------------------------------------------------
-- Row level security on storage.objects is enabled by Supabase; make it explicit and fail loudly if the
-- migration role cannot manage the table.
do $$
begin
...
exception
  when insufficient_privilege then
    raise exception 'cannot manage storage.objects policies'
      using hint = 'The migration role must own storage.objects (or belong to its owner) so bucket access stays defined in migrations.';
end;
$$;
