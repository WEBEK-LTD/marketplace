...
/**
 * The admin app's only reader of the process environment (server runtime only).
...
export function loadServerConfig(source: Readonly<Record<string, string | undefined>> = process.env): NextServerConfig {
  return readNextServerConfig('admin', source);
}
...
export function validateServerConfigAtStartup(): void {
  ...
  console.info(JSON.stringify(configLoadedEvent('admin', NEXT_SERVER_FIELDS)));
}
